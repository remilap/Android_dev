...
import com.remilapointe.country.business.CountryInfo
...
class CountryDetailsActivity : AppCompatActivity(), OnEditorActionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intentThatStartedThisActivity.hasExtra(Intent.EXTRA_TEXT)) {
            val countryInfo = intentThatStartedThisActivity.extras!!.get(Intent.EXTRA_TEXT) as CountryInfo
            var modif = false
            txt_name.setText(countryInfo.abbrev)
            txt_name.setOnEditorActionListener { textView, actionId, event ->
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    countryInfo.abbrev = txt_name.text.toString().trim()
                    toast("modif country name_fr: "+countryInfo.abbrev)
                    modif = true
            ...
            txt_capital.setText(countryInfo.abbrev)
            txt_capital.setOnEditorActionListener { _, actionId, _ ->
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    countryInfo.abbrev = txt_capital.text.toString().trim()
                    toast("modif country capital_fr: "+countryInfo.abbrev)
                    modif = true
                    bt_ok.isEnabled = true
                    return@setOnEditorActionListener true
                }
            ...
            txt_surface.setText(String.format("%,d", countryInfo.surface))
            txt_surface.setOnEditorActionListener { _, actionId, _ ->
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    countryInfo.surface = txt_surface.text.subSequence(0, txt_surface.text.length-4).toString().replace("\\s+", "").toLong()
                    modif = true
                    bt_ok.isEnabled = true
                    return@setOnEditorActionListener true
                }
            ...
            txt_population.setText(String.format("%,d", countryInfo.population))
            txt_population.setOnEditorActionListener { _, actionId, _ ->
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    countryInfo.population = txt_population.text.toString().replace("\\s+", "").toLong()
                    modif = true
                    bt_ok.isEnabled = true
                    return@setOnEditorActionListener true
                }
